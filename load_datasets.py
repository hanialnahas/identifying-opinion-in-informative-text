...
voxpopuli = load_dataset(
    "esb/datasets", "voxpopuli", trust_remote_code=True, token=TOKEN
)
cv_13 = load_dataset(
    "mozilla-foundation/common_voice_13_0", "en", trust_remote_code=True, token=TOKEN
)
amazon_polarity = load_dataset("fancyzhx/amazon_polarity", token=TOKEN)

cv_13 = cv_13.remove_columns(
    [
        "client_id",
        "path",
        "up_votes",
...
for split, num_samples in [
    ("train", 1000000 - len(voxpopuli["train"])),
    ("validation", 16000 - len(voxpopuli["validation"])),
]:
    amazon_polarity[split] = (
        amazon_polarity[split].shuffle(seed=42).select(range(num_samples))
    )
    combined_dataset[split] = concatenate_datasets(
        [amazon_polarity[split], voxpopuli[split]]
    )

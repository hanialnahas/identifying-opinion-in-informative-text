...
class InformativeModel:
    ...
    def __init__(self, token: str = '', dataset = None, wandb_project: str = ''):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_id, quantization_config=bnb_config, device_map={"": 0}, token=self.token)
    ...
    def prepare_dataset(self, dataset):
        def tokenize_function(examples):
            return self.tokenizer(examples["text"], truncation=True, max_length=max_seq_length)
    ...
    def _create_trainer(
        ...
        if tokenize_dataset:
            ...
            eval_dataset = self.prepare_dataset(
                self.get_dataset(
                    self.dataset, "validation", True, 16000
                )
            )
        else:

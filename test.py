...
informative_model = AutoModelForCausalLM.from_pretrained(
    "merged/informative_merged", quantization_config=bnb_config, device_map={"": 0}
)
opinionated_model = AutoModelForCausalLM.from_pretrained(
    "merged/opinionated_merged", quantization_config=bnb_config, device_map={"": 0}
)
...
def get_sentence_probability(sentence):
    ...
    with torch.no_grad():
        outputs_informative = informative_model(inputs)[0]
        outputs_opinionated = opinionated_model(inputs)[0]
    for i, token in enumerate(inputs[0][1:]):
        p2 = (
            torch.nn.functional.softmax(outputs_informative[0, i, :], dim=-1)[
                token
            ].item()
            * 100
        )
        p3 = (
            torch.nn.functional.softmax(outputs_opinionated[0, i, :], dim=-1)[
                token
            ].item()
            * 100
        )
